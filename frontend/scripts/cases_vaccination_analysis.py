...
OUTPUT_DIR = PROJECT_ROOT / "public" / "analysis" / "cases-vaccination"
...
def read_country_rows(csv_path, usecols, countries):
    ...
    for chunk in pd.read_csv(csv_path, usecols=usecols, chunksize=CHUNKSIZE, sep=None, engine="python"):
...
def apply_theme(fig, title, height=800, legend_orientation="h"):
    fig.update_layout(
        ...
        hoverlabel=dict(bgcolor="#fffdf8", bordercolor="#d7cec0", font=dict(color=TEXT)),
        legend=dict(
...
def set_date_axis(fig):
    fig.update_xaxes(range=DATE_AXIS_RANGE, tickformat="%Y", dtick="M12", tick0="2020-01-01")
...
compact = read_country_rows(COMPACT_FILE, usecols=["country", "population"], countries=COUNTRIES)
...
vaccines["people_vaccinated_interpolated"] = pd.to_numeric(vaccines["people_vaccinated_interpolated"], errors="coerce")
...
df["people_vaccinated_interpolated"] = df.groupby("country")["people_vaccinated_interpolated"].ffill().fillna(0)
df["vaccination_rate"] = (df["people_vaccinated_interpolated"] / df["population"] * 100).clip(lower=0, upper=100)

for lag in [0, 14, 28]:
    if lag == 0:
        df[f"new_cases_pm_7d_after_{lag}d"] = df["new_cases_pm_7d"]
    else:
        df[f"new_cases_pm_7d_after_{lag}d"] = df.groupby("country")["new_cases_pm_7d"].shift(-lag)

df["vaccination_stage"] = pd.cut(
    df["vaccination_rate"],
    bins=[-0.01, 20, 60, 100],
    labels=["Low vaccination (<20%)", "Medium vaccination (20%-60%)", "High vaccination (>=60%)"],
)
...
for i, country in enumerate(COUNTRIES):
    ...
    fig1.update_yaxes(title_text="New cases per million<br>7-day average" if col == 1 else "", row=row, col=col, secondary_y=False)
    fig1.update_yaxes(title_text="Vaccination rate (%)" if col == 2 else "", range=[0, 100], row=row, col=col, secondary_y=True)

apply_theme(fig1, "Vaccination Rate vs New COVID-19 Cases in Four Representative Countries", height=960)
set_date_axis(fig1)
...
stage_summary["vaccination_stage_short"] = stage_summary["vaccination_stage"].map(STAGE_SHORT)
...
stage_order = ["Low vaccination (<20%)", "Medium vaccination (20%-60%)", "High vaccination (>=60%)"]
...
for stage in stage_order:
    ...
    fig2.add_trace(
        go.Scatter(
            ...
            textfont=dict(size=12, color=TEXT, family="Inter, Segoe UI, Arial, sans-serif"),
            customdata=np.stack(
                [
                    g["days"],
...
for country in COUNTRIES:
    ...
    for lag in [0, 14, 28]:
        ...
        if len(temp) >= 30 and temp["vaccination_rate"].nunique() > 1 and temp[y_col].nunique() > 1:
...
correlation_summary["pearson_interpretation"] = correlation_summary["pearson_correlation"].apply(interpret_corr)
correlation_summary["spearman_interpretation"] = correlation_summary["spearman_correlation"].apply(interpret_corr)
correlation_summary.to_csv(OUTPUT_DIR / "correlation_summary.csv", index=False)

fig3 = make_subplots(rows=2, cols=2, subplot_titles=COUNTRIES, vertical_spacing=0.16, horizontal_spacing=0.14)
...
for i, country in enumerate(COUNTRIES):
    ...
    fig3.add_trace(
        go.Scatter(
            ...
            mode="markers",
            name=country,
            marker=dict(size=4.6, color=color, opacity=0.36, line=dict(width=0.45, color="rgba(255,255,255,0.55)")),
            showlegend=False,
    ...
    if len(g) >= 30 and g["vaccination_rate"].nunique() > 1:
        ...
        if len(temp_corr) >= 30:
            pearson_value = temp_corr["vaccination_rate"].corr(temp_corr["new_cases_pm_7d_after_28d"], method="pearson")
            spearman_value = safe_spearman_corr(temp_corr["vaccination_rate"], temp_corr["new_cases_pm_7d_after_28d"])
            pearson_text = f"{pearson_value:.3f}" if pd.notna(pearson_value) else "N/A"
            spearman_text = f"{spearman_value:.3f}" if pd.notna(spearman_value) else "N/A"

        g["bin"] = pd.cut(g["vaccination_rate"], bins=np.linspace(0, 100, 11), include_lowest=True)
        binned = (
            g.groupby("bin", observed=True)
            .agg(vaccination_rate=("vaccination_rate", "median"), log_cases_28d=("log_cases_28d", "median"), n=("log_cases_28d", "count"))
            .reset_index()
        )
        ...
        if len(binned) >= 2:
            fig3.add_trace(
                go.Scatter(
                    ...
                    marker=dict(size=7.5, color=PLOT_BG, line=dict(width=2, color=color)),
                    showlegend=False,
    ...
    fig3.update_xaxes(title_text="Vaccination rate (%)", range=[0, 100], row=row, col=col)
    fig3.update_yaxes(title_text="log10(new cases + 1)<br>28 days later" if col == 1 else "", row=row, col=col)

apply_theme(fig3, "Relationship between Vaccination Rate and New Cases after 28 Days", height=940)
...
fig4 = make_subplots(
    ...
    subplot_titles=["New cases per million over time", "Vaccination milestone comparison"],
...
for country in COUNTRIES:
    ...
    for threshold in [20, 60, 80]:
        ...
        if not reached.empty:
            milestones.append({"country": country, "threshold": f"{threshold}%", "threshold_value": threshold, "date": reached.iloc[0]["date"]})
...
for threshold in [20, 60, 80]:
    ...
    fig4.add_trace(
        go.Scatter(
            ...
            marker=dict(size=threshold_sizes[threshold], symbol=threshold_symbols[threshold], color=ACCENT if threshold == 80 else DEEP_GREEN, opacity=0.9, line=dict(width=1.5, color=PLOT_BG)),
            text=g["threshold"],
...
fig4.update_yaxes(title_text="Country", categoryorder="array", categoryarray=COUNTRIES[::-1], row=2, col=1)
fig4.update_xaxes(title_text="Date", range=DATE_AXIS_RANGE, tickformat="%Y", dtick="M12", tick0="2020-01-01", row=1, col=1)
fig4.update_xaxes(title_text="Date when milestone was first reached", range=DATE_AXIS_RANGE, tickformat="%Y", dtick="M12", tick0="2020-01-01", row=2, col=1)

apply_theme(fig4, "Cross-country Comparison: Case Waves and Vaccination Milestones", height=900)
fig4.update_layout(legend_title="Country / milestone", legend=dict(orientation="h", y=1.02, x=1.0, xanchor="right"))
...
for country in COUNTRIES:
    ...
    for stage_name in ["Low vaccination (<20%)", "High vaccination (>=60%)"]:
        g_stage = g_country[g_country["vaccination_stage"] == stage_name].dropna(subset=["new_cases_pm_7d"])
...
fig5 = go.Figure()
for stage in [STAGE_SHORT["Low vaccination (<20%)"], STAGE_SHORT["High vaccination (>=60%)"]]:
...
fig6 = make_subplots(rows=2, cols=2, subplot_titles=COUNTRIES, vertical_spacing=0.16, horizontal_spacing=0.14)
...
for i, country in enumerate(COUNTRIES):
    ...
    for threshold, threshold_color, y_pos_ratio in [(20, "#1f7a6b", 0.92), (60, "#d96b2b", 0.78)]:
    ...
    fig6.update_yaxes(title_text="New cases per million<br>7-day average" if col == 1 else "", row=row, col=col)
